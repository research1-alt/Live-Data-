...
const App: React.FC = () => {
  ...
  const handleNewFrame = useCallback((id: string, dlc: number, data: string[], simulated = false) => {
    ...
    const newFrame: CANFrame = {
      ...
      dlc,
      data,
      timestamp: sessionStartTime ? (perfNow - sessionStartTime) : 0,
  ...
  const saveTraceFile = useCallback(async () => {
    ...
    try {
      ...
      frames.forEach((f, i) => {
        trcContent.push(`${(i+1).toString().padStart(5, ' ')}) ${(f.timestamp/1000).toFixed(1).padStart(13, ' ')} ${f.direction.padStart(13, ' ')} ${f.id.replace('0x', '').padStart(13, ' ')} ${f.dlc.toString().padStart(7, ' ')}   ${f.data.join(' ')}\n`);
      });
